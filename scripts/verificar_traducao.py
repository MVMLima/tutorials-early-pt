#!/usr/bin/env python3
"""
Verifica a integridade da tradução comparando cada episódio em inglês (já convertido para
Quarto, em _source/en-quarto/) com a versão em português entregue pelos agentes.
...
PARES = [
    ("_source/en-quarto/episodios/01-ler-dados.qmd", "episodios/01-ler-dados.qmd"),
    ("_source/en-quarto/episodios/02-limpar-dados.qmd", "episodios/02-limpar-dados.qmd"),
    ("_source/en-quarto/episodios/03-validar-dados.qmd", "episodios/03-validar-dados.qmd"),
    ("_source/en-quarto/episodios/04-agregar-visualizar.qmd", "episodios/04-agregar-visualizar.qmd"),
...
def normalizar(codigo: str) -> str:
    """Remove comentários R e espaços para comparar só o que é código de verdade."""
    limpas = []
...
def checar(en_rel: str, pt_rel: str) -> list:
    ...
    en_txt, pt_txt = en_p.read_text(encoding="utf-8"), pt_p.read_text(encoding="utf-8")
    ...
    en_words = re.compile(
        r"\b(the|and|of|with|from|that|this|which|are|is|to be|you can|we can|"
    ...
    for n, linha in enumerate(pt_txt.split("\n"), 1):
        ...
        if dentro or len(linha) < 60:
            continue
        # ignora os termos originais entre parênteses: "Português (*english term*)"
